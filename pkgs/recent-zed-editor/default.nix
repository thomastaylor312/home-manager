{ lib, rustPlatform, fetchFromGitHub, cmake, copyDesktopItems, curl, perl
, pkg-config, protobuf, fontconfig, freetype, libgit2, openssl, sqlite, zlib
, zstd, alsa-lib, libxkbcommon, wayland, libglvnd, xorg, stdenv, makeFontsConf
, vulkan-loader, envsubst, nix-update-script, cargo-about, versionCheckHook
, buildFHSEnv, cargo-bundle, git, apple-sdk_15, darwinMinVersionHook
, makeBinaryWrapper, nodejs, libGL, libX11, libXext, livekit-libwebrtc, testers
, writableTmpDirAsHomeHook,

withGLES ? false, buildRemoteServer ? true, zed-editor, }:
...
let
  ...
  fhs = { zed-editor, additionalPkgs ? pkgs: [ ], }:
    buildFHSEnv {
...
in rustPlatform.buildRustPackage (finalAttrs: {
  ...
  src = fetchFromGitHub {
    ...
    hash = "sha256-jAnrOrTXP6/WOBBzAMD8S+Soe8dciPSwS1A0aB5Jv8U=";
  };
  ...
  cargoHash = "sha256-dclwpbnyEr1BnyBphz9GDNE2lGKgfNdOXJh6daoPM0c=";
  ...
  buildInputs = [ curl fontconfig freetype libgit2 openssl sqlite zlib zstd ]
  ...
  buildFeatures =
    lib.optionals stdenv.hostPlatform.isDarwin [ "gpui/runtime_shaders" ];
  ...
  env = {
    ...
    ZED_UPDATE_EXPLANATION =
      "Zed has been installed using Nix. Auto-updates have thus been disabled.";
    # Used by `zed --version`
  ...
  '' + lib.optionalString stdenv.hostPlatform.isLinux ''
    ...
    (
      ...
      ${
        lib.getExe envsubst
      } < "crates/zed/resources/zed.desktop.in" > "$out/share/applications/dev.zed.Zed.desktop"
    )
  ...
  meta = {
    ...
    changelog =
      "https://github.com/zed-industries/zed/releases/tag/v${finalAttrs.version}";
